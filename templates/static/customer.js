document.addEventListener('DOMContentLoaded', () => {
    ...
    registerCustomerForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('http://192.168.137.156:5000/registerCustomer', {
                method: 'POST',
                headers: {
    ...
    fetchCustomerForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('http://192.168.137.156:5000/fetchSpecificCustomer', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async function fetchAllCustomers() {
        try {
            const response = await fetch('http://192.168.137.156:5000/fetchAllCustomers', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
