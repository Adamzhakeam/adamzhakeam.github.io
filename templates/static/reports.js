...
document.getElementById('fetchAllSalesBtn').addEventListener('click', () => {
    fetchSales('http://192.168.137.156:5000/fetchAllSalesReports');
});

document.getElementById('fetchSpecificDateBtn').addEventListener('click', () => {
    const date = prompt('Enter the date (YYYY-MM-DD):');
    if (date) {
        fetchSales('http://192.168.137.156:5000/fetchSpecificSalesReports', { saleDate: date });
    }
});
...
document.getElementById('fetchSalesFromToBtn').addEventListener('click', () => {
    ...
    if (startDate && endDate) {
        fetchSales('http://192.168.137.156:5000/fetchSpecificSalesFromToReports', { dateFrom: startDate, dateTo: endDate });
    }
});
...
document.getElementById('generateCSVBtn').addEventListener('click', () => {
    ...
    switch (dateRange) {
        case 'today':
            ...
            endpoint = 'http://192.168.137.156:5000/fetchSpecificSalesReports';
        ...
        case 'thisWeek':
            ...
            endpoint = 'http://192.168.137.156:5000/fetchSpecificSalesFromToReports';
