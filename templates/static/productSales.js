document.addEventListener('DOMContentLoaded', () => {
    ...
    fetchAllSalesBtn.addEventListener('click', () => {
        fetchSales('http://192.168.137.156:5000/fetchAllProductSales');
    });

    fetchSpecificSalesBtn.addEventListener('click', () => {
        const saleDate = prompt('Enter specific sale date (YYYY-MM-DD):');
        if (saleDate) {
            fetchSales('http://192.168.137.156:5000/fetchSpecificProductSales', { saleDate: saleDate });
        }
    });
    ...
    fetchSalesFromToBtn.addEventListener('click', () => {
        ...
        if (dateFrom && dateTo) {
            fetchSales('http://192.168.137.156:5000/fetchSpecificProductSalesFromTo', { dateFrom: dateFrom, dateTo: dateTo });
        }
    });
    ...
    function fetchProductNames(sales) {
        ...
        Promise.all(productIds.map(productId => 
            fetch('http://192.168.137.156:5000/fetchSpecificProductById', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
